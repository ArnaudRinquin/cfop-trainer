...
const ollTwoLookCorners: RawAlgorithm[] = [
  {
...
const pllTwoLookCorners: RawAlgorithm[] = [
  {
    id: 'pll-2l-aa',
...
const pllTwoLookEdges: RawAlgorithm[] = [
  {
    id: 'pll-2l-ua',
...
const RAW_GROUPS: Record<AlgGroupKey, RawAlgorithm[]> = {
  '2look-oll-edges': ollTwoLookEdges,
  '2look-oll-corners': ollTwoLookCorners,
  '2look-pll-corners': pllTwoLookCorners,
  '2look-pll-edges': pllTwoLookEdges,
  'f2l-basic': f2lBasic,
  'oll-full': ollFull,
  'pll-full': pllFull,
};

export const ALG_GROUP_KEYS = Object.keys(RAW_GROUPS) as AlgGroupKey[];

export function getAlgorithmGroup(key: AlgGroupKey, locale: Locale): Algorithm[] {
  return localize(RAW_GROUPS[key], locale) as Algorithm[];
}

export function getAlgorithm(id: string, locale: Locale): Algorithm | undefined {
  for (const key of ALG_GROUP_KEYS) {
    const algs = RAW_GROUPS[key];
    const match = algs.find((a) => a.id === id);
    if (match) return localize(match, locale) as Algorithm;
  }
  return undefined;
}
